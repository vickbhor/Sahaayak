...
NUDGE_REPLIES = {
    "en": (
        "I don't have the complete picture yet, but I can give you a best-guess "
        "assessment with what you've shared so far - or you can tell me a bit more first."
    ),
    "hi": (
        "मुझे अभी पूरी जानकारी नहीं मिली है, लेकिन जो आपने अब तक बताया है उसी के आधार पर मैं "
        "एक अनुमानित आकलन दे सकता हूँ - या आप चाहें तो थोड़ा और बता सकते हैं।"
    ),
    "hinglish": (
        "Mujhe poori tasveer nahi mili, lekin jitna aapne bataya hai usi se best-guess "
        "assessment de sakta hoon - chahen to report bana dun, ya thoda aur bata sakte hain."
    ),
}
...
DISCLAIMER: This is an AI-assisted triage tool and NOT a substitute for professional medical advice."""
        ...
        language_key = (request.preferred_language or "en").lower()
        ...
        if user_turns < MIN_TURNS:
            # Too early to even ask - let the doctor persona keep gathering naturally.
            pass
        else:
            readiness = await assess_conversation_readiness(request.messages)
            has_symptoms = readiness["ready"]
            ...
            if has_symptoms:
                ...
                return TriageResponse(
                    reply=NUDGE_REPLIES.get(language_key, NUDGE_REPLIES["en"]),
                    is_report=False,
                    suggest_report=True,
                )
            elif readiness["next_question"]:
                missing_str = ", ".join(readiness["missing"]) or "more clinical detail"
                focus_line = (
                    f"The information still missing to form an assessment is: {missing_str}. "
                    f"For your one clinical question this turn, ask something close in spirit to: "
                    f"\"{readiness['next_question']}\" - phrased naturally in your own voice and "
                    f"in the patient's language, not copied verbatim."
                )
